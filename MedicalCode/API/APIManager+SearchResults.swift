...
extension APIManager {
    static func getSearchResults(searchData: SearchData, comlpetion: @escaping (_ error: String?, _ doctors: [Clinic]?, _ pagesCount: Int?) -> Void) {
        ...
        Alamofire.request(url, method: .post, parameters: param, encoding: JSONEncoding.default, headers: header).validate()
            .responseObject { (response: DataResponse<SearchResultResponse>) in
                ...
                case .success(_):
                    if let doctors = response.result.value?.data, let pagesCount = response.result.value?.pagesCount {
                        comlpetion(nil, doctors, pagesCount)
                    } else {
